...
The verifier MUST compute hashes from staged blob content in the git index — never from working-tree bytes.

#### Scenario: Working-tree noise does not affect verdicts

- GIVEN a staged set fully consistent with the manifest
- WHEN an unrelated unstaged modification exists in a registered working-tree file
...
3. A new PDF under `bibliografia/` without its converted Markdown counterpart staged and both registered in that same commit.
...
- GIVEN a registered PDF with altered content staged
- WHEN committing without a manifest change
- THEN it is rejected, naming the offending file

#### Scenario: Unpaired new PDF is blocked

- GIVEN a new PDF staged under `bibliografia/`
- WHEN its converted Markdown is absent from the same commit
